...
public class FlyBehaviour : MonoBehaviour {
    ...
    void Update() {
        ...
        Vector3 direction = newPos - _lastPos;                                  // Movement direction

        if (direction.sqrMagnitude > 1e-6f) {
            Quaternion targetRot = Quaternion.LookRotation(direction.normalized, Vector3.up);   // Look at where it's going
            ...
            else transform.rotation = Quaternion.Slerp(transform.rotation, targetRot, speed * dt);
        }
        ...
        // Feathers animation
        float verticalSpeed = direction.y / dt;
